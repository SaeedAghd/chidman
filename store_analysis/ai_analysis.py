...
"""
    ...
    def _create_analysis_prompt(self, store_data: Dict[str, Any]) -> str:
        ...
        prompt = f"""
        شما یک متخصص طراحی فروشگاه و مشاور تجاری با تجربه 20 ساله هستید. شما باید مانند یک دوست صمیمی و مشاور قابل اعتماد با مالک فروشگاه {store_name} صحبت کنید.

        **مهم: در تمام پاسخ خود باید:**
        1. از نام فروشگاه "{store_name}" استفاده کنید
        2. به محصولات، رنگ‌بندی، چیدمان و نورپردازی موجود اشاره کنید
        3. بازار هدف و مشتریان را در نظر بگیرید
        4. مانند یک دوست صمیمی و حرفه‌ای صحبت کنید
        5. تحلیل کاملاً شخصی‌سازی شده و منحصر به فرد ارائه دهید

        **اطلاعات کامل فروشگاه {store_name}:**
        ...
        👥 **رفتار مشتریان {store_name}:**
        - تعداد مشتری روزانه: {daily_customers} نفر
        ...
        **قفسه‌بندی هوشمند {store_name}:**
        [تحلیل ارتفاع قفسه‌ها، دسترسی و دسته‌بندی محصولات]

        ### 💡 تحلیل نورپردازی تخصصی
        **نور عمومی {store_name}:**
        [تحلیل روشنایی کلی و یکنواختی نور در فروشگاه]

        **نور تأکیدی {store_name}:**
        [تحلیل نورپردازی روی محصولات خاص و ویترین‌ها]

        **نورپردازی احساسی {store_name}:**
        [تحلیل نورپردازی متناسب با نوع کسب‌وکار و مشتریان]

        ### 🎨 تحلیل هویت بصری و برندسازی
        **رنگ سازمانی {store_name}:**
        [تحلیل پالت رنگی و هماهنگی با برند]

        **متریال و بافت {store_name}:**
        [تحلیل متریال‌های استفاده شده و انتقال حس برند]

        **نشانه‌گذاری و گرافیک {store_name}:**
        [تحلیل تابلوها، لوگو و علائم راهنما]

        ### 👥 تحلیل تجربه مشتری (Customer Experience)
        **حس پنج‌گانه در {store_name}:**
        - دیداری: [تحلیل رنگ‌ها و نور]
        - شنیداری: [تحلیل موسیقی و صدا]
        - بویایی: [تحلیل رایحه و بو]
        - لامسه: [تحلیل بافت محصولات و دکور]
        - چشایی: [تحلیل پذیرایی و تجربه]

        **راحتی و آرامش {store_name}:**
        [تحلیل فضای نشستن، اتاق پرو و فاصله‌گذاری]

        **تعامل دیجیتال {store_name}:**
        [تحلیل نمایشگرها، QR کدها و تکنولوژی‌های تعاملی]

        ### 🛍️ تحلیل ویترین و نقطه فروش
        **ویترین جذاب {store_name}:**
        [تحلیل ویترین و داستان‌سرایی بصری]

        **ترکیب‌بندی محصولات {store_name}:**
        [تحلیل چینش بر اساس تم رنگی، فصل و ترند]

        **صندوق و خروجی {store_name}:**
        [تحلیل تجربه نهایی خرید و بسته‌بندی]

        ### 👤 تحلیل جزئیات انسانی
        **ارگونومی {store_name}:**
        [تحلیل دسترسی محصولات و راحتی مشتری]

        **راهنمایی روان {store_name}:**
        [تحلیل علائم و جلوگیری از گم‌گشتگی]

        **خدمات انسانی {store_name}:**
        [تحلیل جایگاه مشاوره و پرسنل]

        ### 🛍️ تحلیل محصولات و بازار
        **محصولات {store_data.get('top_products', 'نامشخص')}:**
        [تحلیل محصولات اصلی و نحوه نمایش آن‌ها در {store_name}]

        **بازار هدف {store_name}:**
        [تحلیل تطبیق فروشگاه با بازار هدف {daily_customers} مشتری روزانه]

        ### 💡 توصیه‌های عملی و شخصی‌سازی شده
        [حداقل 10 توصیه خاص برای {store_name}]

        ### 🌈 توصیه‌های تخصصی رنگ‌بندی و چیدمان
        **رنگ‌بندی محصولات {store_name}:**
        [توصیه‌های خاص برای رنگ‌بندی محصولات بر اساس نوع کسب‌وکار {store_type}]

        **چیدمان محصولات برای جلب توجه:**
        [راهکارهای عملی برای چیدمان محصولات که جلب توجه کند]

        **استراتژی‌های جلب توجه مشتری:**
        [تکنیک‌های روانشناسی برای جلب توجه مشتریان در {store_name}]

        ### 🏗️ توصیه‌های معماری فضایی
        **بهبود نقشه حرکتی {store_name}:**
        [توصیه‌های برای ایجاد مسیر روان از ورودی تا نقطه فروش]

        **بهینه‌سازی منطقه داغ {store_name}:**
        [راهکارهای برای قرارگیری محصولات مهم در نقاط پرتردد]

        **قفسه‌بندی هوشمند {store_name}:**
        [توصیه‌های برای ارتفاع، دسترسی و دسته‌بندی قفسه‌ها]

        ### 💡 توصیه‌های نورپردازی تخصصی
        **نورپردازی عمومی {store_name}:**
        [توصیه‌های برای روشنایی یکنواخت و ملایم]

        **نورپردازی تأکیدی {store_name}:**
        [راهکارهای برای تمرکز نور روی محصولات خاص]

        **نورپردازی احساسی {store_name}:**
        [توصیه‌های برای ایجاد فضای گرم و صمیمی]

        ### 🎨 توصیه‌های هویت بصری
        **رنگ سازمانی {store_name}:**
        [توصیه‌های برای پالت رنگی هماهنگ با برند]

        **متریال و بافت {store_name}:**
        [راهکارهای برای انتقال حس برند از طریق متریال]

        **نشانه‌گذاری {store_name}:**
        [توصیه‌های برای تابلوها، لوگو و علائم راهنما]

        ### 👥 توصیه‌های تجربه مشتری
        **حس پنج‌گانه {store_name}:**
        [توصیه‌های برای بهبود تجربه دیداری، شنیداری، بویایی، لامسه و چشایی]

        **راحتی و آرامش {store_name}:**
        [راهکارهای برای فضای نشستن، اتاق پرو و فاصله‌گذاری]

        **تعامل دیجیتال {store_name}:**
        [توصیه‌های برای نمایشگرها، QR کدها و تکنولوژی‌های تعاملی]

        ### 🛍️ توصیه‌های ویترین و نقطه فروش
        **ویترین جذاب {store_name}:**
        [راهکارهای برای داستان‌سرایی بصری و جلب توجه]

        **ترکیب‌بندی محصولات {store_name}:**
        [توصیه‌های برای چینش بر اساس تم رنگی، فصل و ترند]

        **صندوق و خروجی {store_name}:**
        [راهکارهای برای تجربه نهایی خرید و بسته‌بندی]

        ### 👤 توصیه‌های جزئیات انسانی
        **ارگونومی {store_name}:**
        [توصیه‌های برای دسترسی آسان و راحتی مشتری]

        **راهنمایی روان {store_name}:**
        [راهکارهای برای علائم واضح و جلوگیری از گم‌گشتگی]

        **خدمات انسانی {store_name}:**
        [توصیه‌های برای جایگاه مشاوره و پرسنل]

        ### 📈 برنامه بهبود مرحله‌ای
        **مرحله 1 (هفته 1-2) - اقدامات فوری {store_name}:**
        - بهبود نورپردازی عمومی و تأکیدی
        - بهینه‌سازی چیدمان محصولات پرفروش
        - ایجاد نقاط کانونی جذاب
        - بهبود نشانه‌گذاری و علائم راهنما
        - تنظیم موسیقی و محیط صوتی
        - بهینه‌سازی نقشه حرکتی مشتری
        - شناسایی و بهبود منطقه داغ (Hot Zone)
        - قفسه‌بندی هوشمند و ارگونومی
        - نورپردازی احساسی متناسب با نوع کسب‌وکار
        - بهبود تجربه پنج‌گانه مشتری

        **مرحله 2 (هفته 3-4) - بهبودهای کوتاه‌مدت {store_name}:**
        - هویت بصری و برندسازی
        - متریال و بافت متناسب با برند
        - تعامل دیجیتال و تکنولوژی‌های مدرن
        - ویترین و نقطه فروش حرفه‌ای
        - خدمات انسانی و مشاوره تخصصی
        - بهبود تجربه نهایی خرید و بسته‌بندی
        - ایجاد فضای راحت و آرامش‌بخش
        - بهینه‌سازی دسترسی و راحتی مشتری
        - راهنمایی روان و جلوگیری از گم‌گشتگی
        - قرارگیری مناسب پرسنل برای خدمت‌رسانی

        **مرحله 3 (ماه 2-3) - بهبودهای بلندمدت {store_name}:**
        - پیاده‌سازی کامل سیستم مدیریت مشتری
        - بهینه‌سازی فرآیندهای فروش و بازاریابی
        - ایجاد سیستم بازخورد و بهبود مستمر
        - پیاده‌سازی تکنولوژی‌های پیشرفته
        - آموزش پرسنل و بهبود خدمات
        - ایجاد سیستم مدیریت موجودی هوشمند
        - بهینه‌سازی هزینه‌ها و افزایش سودآوری
        - ایجاد سیستم وفاداری مشتری
        - پیاده‌سازی استراتژی‌های بازاریابی دیجیتال
        - ایجاد سیستم مدیریت کیفیت و استانداردها

        ### 🎯 پیش‌بینی نتایج
        **نتایج کوتاه‌مدت (1-2 ماه) برای {store_name}:**
        - افزایش 15-25% در فروش روزانه
        - بهبود تجربه مشتری و رضایت‌مندی
        - افزایش زمان ماندگاری مشتری در فروشگاه
        - بهبود کارایی پرسنل و کاهش استرس
        - افزایش فروش محصولات خاص و پرفروش

        **نتایج میان‌مدت (3-6 ماه) برای {store_name}:**
        - افزایش 30-50% در فروش کل
        - بهبود تصویر برند و شهرت
        - افزایش وفاداری مشتریان
        - کاهش هزینه‌های عملیاتی
        - بهبود سودآوری و بازگشت سرمایه

        **نتایج بلندمدت (6-12 ماه) برای {store_name}:**
        - تبدیل شدن به مرجع در منطقه
        - افزایش سهم بازار
        - ایجاد سیستم مدیریت مشتری پیشرفته
        - بهبود مستمر و نوآوری مداوم
        - ایجاد مزیت رقابتی پایدار

        ### 💰 تأثیر بر فروش
        **تأثیر بر فروش روزانه {daily_sales} تومان {store_name}:**
        - افزایش فروش محصولات پرفروش: +20-30%
        - افزایش فروش محصولات جدید: +40-60%
        - بهبود فروش متقابل: +25-35%
        - افزایش فروش آنی: +15-25%
        - بهبود فروش فصلی: +30-50%

        **تأثیر بر هزینه‌ها:**
        - کاهش هزینه‌های بازاریابی: -15-20%
        - بهبود کارایی پرسنل: +20-30%
        - کاهش ضایعات و خرابی: -10-15%
        - بهبود مدیریت موجودی: +15-25%

        ### 📊 معیارهای موفقیت
        **معیارهای کمی برای {store_name}:**
        - افزایش فروش روزانه: هدف +25%
        - افزایش تعداد مشتریان: هدف +20%
        - بهبود زمان ماندگاری: هدف +30%
        - افزایش فروش به ازای متر مربع: هدف +35%
        - بهبود نرخ تبدیل: هدف +40%

        **معیارهای کیفی برای {store_name}:**
        - رضایت‌مندی مشتریان: هدف 90%+
        - رضایت‌مندی پرسنل: هدف 85%+
        - بهبود تصویر برند: هدف 80%+
        - کاهش شکایات: هدف -50%
        - افزایش توصیه‌های مثبت: هدف +60%

        **نکته مهم:** در تمام پاسخ خود از نام "{store_name}" و جزئیات خاص آن استفاده کنید تا مالک احساس کند واقعاً در مورد فروشگاه خودش صحبت می‌کنید.
        """
