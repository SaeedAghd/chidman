...
تیم تحلیل چیدمانو
    ...
    def _generate_analysis_text(self, store_name: str, store_type: str, overall_score: int, action_plan: Dict[str, Any]) -> str:
        """تولید تحلیل دقیق و حرفه‌ای - دستیار چیدمان فروشگاه"""
        
        # تبدیل نوع فروشگاه به فارسی زیبا
        store_type_persian = self._convert_store_type_to_persian(store_type)
        
        # تولید تحلیل دقیق و حرفه‌ای که دقیقاً همان انتظارات کاربر را برآورده کند
        analysis_text = f"""
# 🎯 تحلیل دقیق و حرفه‌ای فروشگاه {store_name}
## دستیار چیدمان فروشگاه - چیدمانو
...
در ادامه، **تحلیل دقیق و حرفه‌ای** فروشگاه {store_name} ارائه می‌گردد که بر اساس اصول علمی و تجربیات موفق تهیه شده است.

**مشخصات فروشگاه:**
• نام: {store_name}
• نوع فعالیت: {store_type_persian}
• امتیاز فعلی: {overall_score} از 100
• وضعیت: نیاز به بهینه‌سازی دارد
...
**دستیار حرفه‌ای چیدمان فروشگاه‌ها - چیدمانو**
    ...
    def _create_current_layout_diagram(self, store_name: str, store_type: str, store_size: str) -> str:
        """ایجاد تصویر چیدمان فعلی"""
        return f"""
# 🏪 چیدمان فعلی فروشگاه {store_name}

## وضعیت موجود (قبل از بهینه‌سازی)
...
### مشکلات چیدمان فعلی:
• محصولات به صورت تصادفی چیده شده‌اند
• مسیر مشتری مشخص نیست
• نقاط جذب وجود ندارد
• نورپردازی یکنواخت و ضعیف است
• محصولات پرفروش در ارتفاع نامناسب قرار دارند
• فاصله بین قفسه‌ها کم است
• عدم وجود ویترین جذاب
...
- کاهش نرخ تبدیل مشتری
    ...
    def _create_optimized_layout_diagram(self, store_name: str, store_type: str, store_size: str) -> str:
        """ایجاد تصویر چیدمان بهینه"""
        return f"""
# 🎯 چیدمان بهینه فروشگاه {store_name}

## وضعیت بهینه (بعد از بهینه‌سازی)
...
### مزایای چیدمان بهینه:
• محصولات بر اساس اصول علمی چیده شده‌اند
• مسیر مشتری به شکل U طراحی شده است
• نقاط جذب در ورودی و نقاط پرتردد قرار دارند
• نورپردازی تاکیدی روی محصولات ویژه
• محصولات پرفروش در ارتفاع چشم قرار دارند
• فاصله مناسب بین قفسه‌ها
• ویترین جذاب در ورودی
